...
async function handleFilter(){
  ...
  else if (jobs) afterFilter = await Alljobs.filter(job =>{ return job.type.toLowerCase() === 'job'})
  else if(internship) afterFilter = await Alljobs.filter(job =>{ return job.type.toLowerCase() === 'internship'})

  
  if(categoryIndex !== "0"){
    afterFilter = await afterFilter.filter(job =>{ return job.category === category})
  } 
